<script lang="ts">
	...
	function createChart(node: HTMLDivElement, chartArguments) {
		...
		let viz1 = chartArguments.viz[0];
		let viz2 = chartArguments.viz[1];

		viz1 = viz1.width(node.clientWidth).height(node.clientHeight);

		viz2 = viz2.width(node.clientWidth).height(node.clientHeight / 2);

		vl.vconcat(viz1, viz2)
